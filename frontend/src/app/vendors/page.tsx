...
type Vendor = {
  ...
  category: Category;
  isActive: boolean;
  // The name is not stored on-chain, so we'll use the address as a fallback
  name: string; 
};
...
function useFamilyVendors() {
  ...
  // Explanation: How can we read from a public mapping like `familyVendors`?
  // When you declare a mapping as `public` in Solidity, the compiler automatically
  // generates a "getter" function for it. This function takes the mapping keys as arguments
  // and returns the value. The `useReadContracts` hook from wagmi calls this
  // auto-generated getter function for each vendor address to fetch its details.
...
// --- UI COMPONENT: AddVendorModal ---
const AddVendorModal = ({ isOpen, onClose, onAddVendor, isProcessing, statusMessage }: AddVendorModalProps) => {
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="mb-6">
                ...
                >
                  ...
                  {CATEGORIES.map((cat, index) => <option key={cat} value={index}>{cat}</option>)}
                </select>
              </div>
...
export default function Vendors() {
  ...
  const statusMessage = isPending ? 'Submitting...' : isConfirming ? 'Confirming...' : 'Add Vendor';
      ...
      <div className="container mx-auto px-6 py-8">
          ...
          {filteredVendors?.map((vendor) => (
            <div key={vendor.address} className="bg-white/10 backdrop-blur-lg rounded-2xl border border-white/20 p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <div className={`flex items-center gap-1 px-2 py-1 rounded-lg text-sm ${vendor.isActive ? 'bg-green-500/20 text-green-400' : 'bg-red-500/20 text-red-400'}`}>
              ...
              <div className="space-y-3">
                <div className="flex justify-between">
                  ...
                  <span className="text-white text-sm font-mono">{vendor.address.slice(0, 6)}...{vendor.address.slice(-4)}</span>
                </div>
              </div>
              <div className="mt-4 pt-4 border-t border-white/10">
                <div className="flex gap-2">
                  <button className="flex-1 bg-slate-600 hover:bg-slate-700 text-white px-4 py-2 rounded-lg transition-colors">Edit</button>
                  <button 
        ...
        {!loading && filteredVendors?.length === 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="text-center py-12">
            ...
            <button onClick={() => setShowAddForm(true)} className="bg-blue-600 hover:bg-blue-700 text-white px-6 py-3 rounded-xl transition-colors">
              Add Vendor
            </button>
          </motion.div>
        )}
        ...
        />
        {error && <p className="text-red-400 mt-4 text-center fixed bottom-4 left-1/2 -translate-x-1/2 bg-slate-800 p-3 rounded-lg">Error: {error.message}</p>}
      </div>
    </div>
  );
}
